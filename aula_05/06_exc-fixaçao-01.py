...
for i in range (len(alune)):
    if media[i] >= 7:
        print(f'O(A) aluno(a) {alune[i]}, foi {situacao[0]}: média {media[i]}')
    elif media[i] < 5:
        print(f'O(A) aluno(a) {alune[i]}, foi {situacao[1]}:  média {media[i]}')
    else:
        print(f'O(A) aluno(a) {alune[i]}, está {situacao[2]}: média {media[i]}')
